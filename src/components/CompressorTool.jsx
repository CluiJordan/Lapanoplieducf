  ...
  const processFile = async () => {
    ...
    const smartCompress = async (imgSource, budget) => {
        ...
        for (let i = 0; i < 5; i++) { 
            ...
            const b = await generateBlob(imgSource, 1.0, mid);
            if (b.size <= budget) { best = b; minQ = mid; } else { maxQ = mid; }
        }
        if (!best) {
            let s = 0.8;
            while (s > 0.2) {
                const b = await generateBlob(imgSource, s, 0.4);
                if (b.size <= budget) { best = b; break; }
                s *= 0.7;
            }
        }
    ...
    setTimeout(async () => {
        try {
            ...
            if (file.type.startsWith('image/')) {
                ...
                finalBlob = await smartCompress(img, targetBytes);
            } else if (file.name.toLowerCase().endsWith('.pdf')) {
                const pdf = await pdfjsLibRef.current.getDocument(await file.arrayBuffer()).promise;
                const newPdf = new jsPDF(); newPdf.deletePage(1);
                const budgetPerPage = Math.floor(targetBytes / pdf.numPages) - 1024;
                ...
                for(let i=1; i<=pdf.numPages; i++){
                    ...
                    await new Promise(r => img.onload = r);
                    const pBlob = await smartCompress(img, budgetPerPage);
                    const w = vp.width * 0.13; const h = vp.height * 0.13;
                    newPdf.addPage([w, h]);
                    newPdf.addImage(new Uint8Array(await pBlob.arrayBuffer()), 'JPEG', 0, 0, w, h);
  ...
  return (
    <div className="animate-in slide-in-from-right-4">
        <div className="flex items-center gap-2 mb-6">
            <button onClick={onBack} className="p-2 bg-white rounded-full border border-slate-100 active:scale-90 transition-all"><ChevronLeft className="w-5 h-5" /></button>
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Compresseur</h2>
        </div>

        <div className="max-w-xl mx-auto bg-white rounded-[40px] shadow-2xl border border-slate-50 p-10">
            {!file ? (
                <label className="flex flex-col items-center justify-center h-52 border-2 border-dashed border-indigo-50 rounded-[30px] cursor-pointer hover:bg-indigo-50/30 transition-all">
                    <Upload className="w-8 h-8 text-indigo-200 mb-3" />
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Glisser un fichier ici</p>
                ...
                <div className="space-y-8">
                    ...
                    {!downloadUrl && !isProcessing && (
                        <div className="space-y-6">
                            <div className="px-2">
                                ...
                                <input type="range" min="50" max="1000" step="10" value={targetSizeKB} onChange={(e) => setTargetSizeKB(e.target.value)} className="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                                <div className="text-center mt-4 font-mono font-black text-indigo-600 text-2xl">{targetSizeKB} Ko</div>
                            </div>
                            <button onClick={processFile} className="w-full py-5 bg-indigo-600 text-white rounded-[25px] font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 active:scale-95 transition-all">Compresser</button>
                        </div>
                    )}
                    ...
                    {downloadUrl && (
                        <div className="bg-indigo-900 rounded-[35px] p-8 text-center text-white shadow-2xl animate-in zoom-in-95">
                            <CheckCircle className="w-10 h-10 text-indigo-300 mx-auto mb-4" />
