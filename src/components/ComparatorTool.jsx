...
const ComparatorTool = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-zinc-950 overflow-hidden transition-colors duration-300">

      {/* HEADER */}
      <div className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 h-16 px-6 flex items-center justify-between shrink-0 z-30 transition-colors">
      ...
      <div className="flex-1 overflow-auto p-5 md:p-8 flex flex-col items-center custom-scrollbar">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-4xl mb-7">
          ...
          <label className={`flex flex-col items-center text-center p-7 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${
            ...
            {file1 ? (
              <>
                <p className="font-bold text-slate-800 dark:text-zinc-100 text-sm truncate max-w-full mb-1">{file1.name}</p>
                <p className="text-xs text-slate-400 dark:text-zinc-500 mb-4">{data1.length} lignes détectées</p>
              ...
              <>
                <p className="font-bold text-slate-700 dark:text-zinc-300 text-sm mb-1">Fichier Référence</p>
                <p className="text-xs text-slate-400 dark:text-zinc-500 mb-4">Fichier A</p>
          ...
          <label className={`flex flex-col items-center text-center p-7 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${
            ...
            {file2 ? (
              <>
                <p className="font-bold text-slate-800 dark:text-zinc-100 text-sm truncate max-w-full mb-1">{file2.name}</p>
                <p className="text-xs text-slate-400 dark:text-zinc-500 mb-4">{data2.length} lignes détectées</p>
              ...
              <>
                <p className="font-bold text-slate-700 dark:text-zinc-300 text-sm mb-1">Fichier Comparé</p>
                <p className="text-xs text-slate-400 dark:text-zinc-500 mb-4">Fichier B</p>
                <span className="text-[10px] font-black uppercase tracking-widest bg-slate-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-2 rounded-lg">
                  Importer B
                </span>
              </>
        ...
        {data1.length > 0 && data2.length > 0 && !comparisonResult && (
          <div className="w-full max-w-4xl bg-white dark:bg-zinc-900 p-7 rounded-2xl shadow-xl border border-indigo-100 dark:border-indigo-900/60 mb-7 animate-in">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-zinc-800">
              ...
              <h3 className="font-black text-sm text-slate-800 dark:text-zinc-100 uppercase tracking-wider">Configuration</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                ...
                {[
                  ...
                  <div key={label}>
                    ...
                    <select
                      ...
                      className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-sm text-slate-800 dark:text-zinc-200 outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-all"
                    >
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-zinc-400 mb-1.5">Colonne Classe</label>
                  <select
                    value={classCol1}
                ...
                {availableClasses.length > 0 && (
                  <div className="bg-slate-50 dark:bg-zinc-800/60 p-3 rounded-xl border border-slate-100 dark:border-zinc-700 max-h-40 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {availableClasses.map(cls => (
                        <label key={cls} className="flex items-center gap-2 cursor-pointer hover:bg-white dark:hover:bg-zinc-700 p-1.5 rounded-lg transition-colors">
                          ...
                          <span className="text-xs font-medium text-slate-700 dark:text-zinc-300 truncate">{cls}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex justify-center pt-4 border-t border-slate-100 dark:border-zinc-800">
              <button
          ...
          <div className="w-full max-w-6xl animate-in">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div
                ...
                className={`cursor-pointer rounded-2xl p-5 text-center border-2 transition-all ${activeTab === 'missing' ? 'bg-red-500 border-red-500 text-white shadow-xl shadow-red-200 dark:shadow-red-900/30' : 'bg-white dark:bg-zinc-900 border-red-100 dark:border-red-900/40 hover:border-red-300'}`}
              >
              ...
              <div
                ...
                className={`cursor-pointer rounded-2xl p-5 text-center border-2 transition-all ${activeTab === 'surplus' ? 'bg-orange-500 border-orange-500 text-white shadow-xl shadow-orange-200 dark:shadow-orange-900/30' : 'bg-white dark:bg-zinc-900 border-orange-100 dark:border-orange-900/40 hover:border-orange-300'}`}
              >
              ...
              <div
                ...
                className={`cursor-pointer rounded-2xl p-5 text-center border-2 transition-all ${activeTab === 'common' ? 'bg-emerald-500 border-emerald-500 text-white shadow-xl shadow-emerald-200 dark:shadow-emerald-900/30' : 'bg-white dark:bg-zinc-900 border-emerald-100 dark:border-emerald-900/40 hover:border-emerald-300'}`}
              >
            ...
            {/* TABLE CARD */}
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xl overflow-hidden flex flex-col" style={{ height: 460 }}>
              {/* Table toolbar */}
              <div className="p-4 border-b border-slate-100 dark:border-zinc-800 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between bg-slate-50/80 dark:bg-zinc-950/40">
                <div className="flex items-center gap-2 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl px-3 py-2 flex-1 max-w-xs">
                  ...
                  <input
                    ...
                    className="bg-transparent outline-none text-xs text-slate-700 dark:text-zinc-300 placeholder-slate-400 w-full"
                  />
                </div>
                <div className="flex gap-2 items-center">
                  <button
                    onClick={() => { setComparisonResult(null); setSearchQuery(''); }}
                    className="px-3 py-2 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-500 dark:text-zinc-400 rounded-xl text-[10px] font-black uppercase transition-colors"
                  >
                    Nouvelle recherche
                  </button>
                  <button
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="sticky top-0 bg-white dark:bg-zinc-900 z-10">
                    <tr>
                      {['#', 'Matricule', 'Nom', 'Prénoms', 'Classe'].map(h => (
                        <th key={h} className="p-3 text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-500 border-b border-slate-200 dark:border-zinc-800 first:w-12 first:text-center">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {filteredRows.map((row, idx) => {
                      const mapping = getCurrentMapping();
                      return (
                        <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors border-b border-slate-50 dark:border-zinc-800">
                          <td className="p-3 text-xs text-slate-400 dark:text-zinc-600 text-center">{idx + 1}</td>
                          <td className="p-3 text-xs font-mono font-bold text-slate-800 dark:text-zinc-200">{row[mapping.matricule] || '—'}</td>
                          <td className="p-3 text-xs text-slate-700 dark:text-zinc-300">{row[mapping.nom] || '—'}</td>
                          <td className="p-3 text-xs text-slate-600 dark:text-zinc-400">{row[mapping.prenom] || '—'}</td>
                          <td className="p-3 text-xs font-semibold text-slate-600 dark:text-zinc-400">{row[mapping.classe] || '—'}</td>
                        </tr>
                      );
                    })}
                    {filteredRows.length === 0 && (
                      <tr>
                        <td colSpan="5" className="p-16 text-center text-slate-400 dark:text-zinc-600 text-sm">
              ...
              {/* Count footer */}
              <div className="px-4 py-2 border-t border-slate-100 dark:border-zinc-800 bg-slate-50/80 dark:bg-zinc-950/40 text-[10px] font-bold text-slate-400 dark:text-zinc-600 uppercase">
