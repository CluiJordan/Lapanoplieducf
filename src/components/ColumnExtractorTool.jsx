...
const ColumnExtractorTool = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {/* HEADER TOOLBAR */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-5 flex items-center justify-between h-16 shrink-0 z-20 transition-colors">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-500 dark:text-slate-400 transition-all active:scale-95">
          ...
          {file && (
            <>
              ...
              <div className="hidden md:flex items-center gap-2">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">Ligne titres</span>
                <input
                  type="number" min="1"
                  value={headerRowIndex}
                  onChange={(e) => setHeaderRowIndex(parseInt(e.target.value) || 1)}
                  className="w-12 py-1 text-center border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-xs font-bold text-orange-600 dark:text-orange-400 outline-none focus:ring-2 focus:ring-orange-400/30 focus:border-orange-400 transition-all"
                />
                <button onClick={() => parseFiledata(rawData, headerRowIndex)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 dark:text-slate-400 transition-colors">
        ...
        <div className="flex items-center gap-2">
          {!file ? (
            ...
            <>
              <button onClick={() => exportData('csv')} disabled={!selectedHeaders.length} className="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-black uppercase rounded-xl shadow-sm disabled:opacity-40 flex items-center gap-1.5 transition-all active:scale-95">
      ...
      {!file ? (
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400">
          <label className="cursor-pointer flex flex-col items-center group">
            <div className="w-24 h-24 bg-white dark:bg-slate-900 rounded-3xl flex items-center justify-center mb-6 shadow-xl border border-slate-200 dark:border-slate-800 group-hover:border-orange-300 dark:group-hover:border-orange-700 transition-all animate-in">
              <Grid className="w-10 h-10 text-slate-200 dark:text-slate-700 group-hover:text-orange-400 transition-colors" />
            </div>
            <p className="font-black text-sm uppercase tracking-widest text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors">
              Charger un fichier Excel
            </p>
            <p className="text-xs text-slate-300 dark:text-slate-600 mt-1">.xlsx ou .xls</p>
            <input type="file" accept=".xlsx,.xls" onChange={handleFileUpload} className="hidden" />
          </label>
        </div>
      ) : (
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* LEFT PANEL */}
          <div className="w-60 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0 transition-colors">
            <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/40">
              <span className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest">Colonnes</span>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
              ...
              {allHeaders.length === 0 && (
                <p className="text-xs text-slate-300 dark:text-slate-600 text-center py-8">Aucune colonne</p>
              )}
            </div>
            <div className="px-3 py-2 border-t border-slate-100 dark:border-slate-800 text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase">
          ...
          <div className="flex-1 flex flex-col bg-slate-100 dark:bg-slate-950 overflow-hidden">
            <div className="flex-1 overflow-auto custom-scrollbar m-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 shadow-sm rounded-xl">
              <table className="w-full text-left border-collapse table-fixed">
                <thead className="bg-slate-50 dark:bg-slate-800/80 sticky top-0 z-10">
                  <tr>
                    <th className="w-10 bg-slate-100 dark:bg-slate-900 border-r border-b border-slate-200 dark:border-slate-700 text-center text-[10px] font-bold text-slate-400 dark:text-slate-500 p-2">#</th>
                    {selectedHeaders.length === 0 && (
                      <th className="p-8 text-center text-slate-300 dark:text-slate-600 text-xs font-normal italic border-none">
                        Cochez des colonnes à gauche pour construire votre export…
                      </th>
                    )}
                    {selectedHeaders.map((h, i) => (
                      <th key={i} className="px-3 py-2 border-b border-r border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 w-36 truncate group bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                        <div className="flex justify-between items-center w-full">
                          <span className="truncate pr-2">{h}</span>
                          <div className="flex opacity-0 group-hover:opacity-100 shrink-0 bg-slate-200 dark:bg-slate-600 rounded overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {previewData.map((row, rIdx) => (
                    <tr key={rIdx} className="hover:bg-orange-50/20 dark:hover:bg-orange-900/5 transition-colors">
                      <td className="bg-slate-50 dark:bg-slate-800/40 border-r border-slate-200 dark:border-slate-700 text-center text-[10px] text-slate-400 dark:text-slate-500 font-mono font-bold p-2">
                        {rIdx + 1}
                      </td>
                      {selectedHeaders.map((h, cIdx) => (
                        <td key={cIdx} className="px-3 py-2 text-xs text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800 truncate font-mono">
                  ...
                  {previewData.length === 0 && selectedHeaders.length > 0 && (
                    <tr>
                      <td colSpan={selectedHeaders.length + 1} className="p-8 text-center text-xs text-slate-300 dark:text-slate-600">
            ...
            <div className="h-9 bg-slate-200 dark:bg-slate-800 border-t border-slate-300 dark:border-slate-700 flex items-center px-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase justify-between shrink-0">
      ...
      {isAddColModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-7 w-full max-w-sm border border-slate-200 dark:border-slate-700 animate-in">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xs font-black text-slate-700 dark:text-slate-200 uppercase tracking-widest">Ajouter une colonne vide</h3>
              <button onClick={() => setIsAddColModalOpen(false)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <input
              ...
              className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20 mb-5 transition-all text-slate-800 dark:text-slate-200 placeholder-slate-400"
            />
            <div className="flex gap-2 justify-end">
              <button onClick={() => setIsAddColModalOpen(false)} className="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-black uppercase transition-colors">
                Annuler
              </button>
